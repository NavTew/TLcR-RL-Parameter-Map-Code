function Y = Reshape3D_20Connection_Spatial(X,B,stepsize,padpixel,c)
...
for i = -padpixel:stepsize:padpixel
    for j = -padpixel:stepsize:padpixel
        if i==0&j==0
            ...
            temp = [temp;repmat(c*[abs(j) abs(i)]',1,size(X,3))];% sombine the spatial feature
        elseif B(1)+i>0 && B(1)+i+patch_size-1<=size(X,1)&&B(3)+j>0 && B(3)+j+patch_size-1<=size(X,2)
            ...
            tX = [tX;repmat(c*[abs(j) abs(i)]',1,size(X,3))]; % sombine the spatial feature
            Y = [Y tX];
        end
    end
end
